...
* Note that a pointer is constrained to point to a particular kind of object: every pointer points to a specific data type. (one exception: a **pointer to void** is used to hold any type of pointer but cannot be dereferenced itself.)
...
| C_Header              | C++_Header      |   C_Standard   | Note |
| :-------------------- | :-------------- | :------------: | :--- |
| \<**assert.h**\>      | \<**cassert**\> |                | Conditionally compiled macro that compares its argument to zero |
| \<**complex.h**\>     |                 | since **C99**  | Complex number arithmetic |
| \<**ctype.h**\>       | \<**cctype**\>  |                | Functions to determine the type contained in character data |
| \<**errno.h**\>       | \<**cerrno**\>  |                | Macros reporting error conditions
| \<**fenv.h**\>        |                 | since **C99**  | Floating-point environment |
| \<**float.h**\>       | \<**cfloat**\>  |                | Limits of float types |
| \<**inttypes.h**\>    |                 | since **C99**  | Format conversion of integer types |
| \<**iso646.h**\>      | \<**ciso646**\> | since **C95**  | Alternative operator spellings |
| \<**limits.h**\>      | \<**climits**\> |                | Sizes of basic types |
| \<**locale.h**\>      | \<**clocale**\> |                | Localization utilities |
| \<**math.h**\>        | \<**cmath**\>   |                | Common mathematics functions |
| \<**setjmp.h**\>      | \<**csetjmp**\> |                | Nonlocal jumps |
| \<**signal.h**\>      | \<**csignal**\> |                | Signal handling |
| \<**stdalign.h**\>    |                 | since **C11**  | alignas and alignof convenience macros |
| \<**stdarg.h**\>      | \<**cstdarg**\> |                | Variable arguments |
| \<**stdatomic.h**\>   |                 | since **C11**  | Atomic types |
| \<**stdbool.h**\>     |                 | since **C99**  | Boolean type |
| \<**stddef.h**\>      | \<**cstddef**\> |                | Common macro definitions |
| \<**stdint.h**\>      |                 | since **C99**  | Fixed-width integer types |
| \<**stdio.h**\>       | \<**cstdio**\>  |                | Input/output |
| \<**stdlib.h**\>      | \<**cstdlib**\> |                | General utilities: memory management, program utilities, string conversions, random numbers |
| \<**stdnoreturn.h**\> |                 | since **C11**  | noreturn convenience macros |
| \<**string.h**\>      | \<**cstring**\> |                | String handling |
| \<**tgmath.h**\>      |                 | since **C99**  | Type-generic math (macros wrapping **math.h** and **complex.h** |
| \<**threads.h**\>     |                 | since **C11**  | Thread library |
| \<**time.h**\>        | \<**ctime**\>   |                | Time/date utilities |
| \<**uchar.h**\>       |                 | since **C11**  | UTF-16 and UTF-32 character utilities |
| \<**wchar.h**\>       | \<**cwchar**\>  | since **C95**  | Extended multibyte and wide character utilities |
| \<**wctype.h**\>      | \<**cwctype**\> | since **C95**  | Wide character classification and mapping utilities |
...
![ctype.h](/assets/ctype.h.png)

### uchar.h

**Macros**

| Types           | Standard  | Description |
| :-------------- | :-------- | :---------- |
| __STDC_UTF_16__ | since C11 | ndicates that UTF-16 encoding is used by ```mbrtoc16()``` and ```c16rtomb()``` |
| __STDC_UTF_32__ | since C11 | indicates that UTF-32 encoding is used by ```mbrtoc32()``` and ```c32rtomb()``` |
...
size_t c16rtomb(char* s, char16_t c16, mbstate_t* ps);  // since C11

size_t mbrtoc32(char32_t* pc32, const char* s, size_t n, mbstate_t* ps);  // since C11
...
| Macros    | Standard  | Description |
| :-------- | :-------- | :---------- |
| WEOF      | since C95 | a non-character value of type wint_t used to indicate errors |
| WCHAR_MIN | since C95 | the smallest valid value of wchar_t |
| WCHAR_MAX | since C95 | the largest valid value of wchar_t |
...
long long wcstoll(const wchar_t* str, wchar_t** str_end, int base); // since C99
...
errno_t  wcscpy_s(wchar_t *restrict dest, rsize_t destsz, const wchar_t *restrict src);  // since C11

wchar_t *wcsncpy(wchar_t* dest, const wchar_t* src, size_t count);  // since C95, until C99
wchar_t *wcsncpy(wchar_t *restrict dest, const wchar_t *restrict src, size_t n);  // since C99
...
errno_t  wcsncat_s(wchar_t *restrict dest, rsize_t destsz, const wchar_t *restrict src, rsize_t count);  // since C11
...
```
size_t wcslen(const wchar_t *str);  // since C95
size_t wcsnlen_s(const wchar_t *str, size_t strsz);  // since C11
...
wchar_t* wcschr(const wchar_t* str, wchar_t ch);  // since C95
wchar_t* wcsrchr(const wchar_t* str, wchar_t ch); // since C95
...
wchar_t* wcspbrk(const wchar_t* dest, const wchar_t* str);  // since C95
wchar_t* wcsstr(const wchar_t* dest, const wchar_t* src);   // since C95
...
wchar_t* wcstok_s(wchar_t *restrict str, rsize_t *restrict strmax,
                  const wchar_t *restrict delim, wchar_t **restrict ptr);  // since C11
```
...
errno_t wmemcpy_s(wchar_t *restrict dest, rsize_t destsz,
                  const wchar_t *restrict src, rsize_t count);  // since C11

wchar_t* wmemmove(wchar_t *dest, const wchar_t* src, size_t count);  // since C95
errno_t  wmemmove_s(wchar_t *dest, rsize_t destsz, const wchar_t *src, rsize_t count);  // since C11

int wmemcmp(const wchar_t *lhs, const wchar_t *rhs, size_t count); // since C95
wchar_t *wmemchr(const wchar_t *ptr, wchar_t ch, size_t count);    // since C95
...
| Types     | Standard  | Description |
| :-------- | :-------- | :---------- |
| wint_t    | since C95 | integer type that can hold any valid wide character and at least one more value |
| wctrans_t | since C95 | scalar type that holds locale-specific character mapping |
| wctype_t  | since C95 | scalar type that holds locale-specific character classification |
...
| Value of *str* in *towctrans()* | Effect |
| :-----------------------------: | :----- |
...
Note: ```iswdigit()``` and ```iswxdigit()``` are the only standard wide character classification functions that are not affected by the currently installed C locale.
...
errno_t memset_s(void *dest, rsize_t destsz, int ch, rsize_t count);  // since C11
...
errno_t strerror_s(char *buf, rsize_t bufsz, errno_t errnum);  // since C11
size_t  strerrorlen_s(errno_t errnum);  // since C11
```
...
#define FE_ALL_EXCEPT   FE_DIVBYZERO | FE_INEXACT  | \         // since C99
...
int fegetexceptflag(fexcept_t* flagp, int excepts);        // since C99
int fesetexceptflag(const fexcept_t* flagp, int excepts);  // since C99

int fesetround(int round);  // since C99
int fegetround();           // since C99

int fegetenv(fenv_t* envp);        // since C99
int fesetenv(const fenv_t* envp);  // since C99
...
errno_t freopen_s(FILE *restrict *restrict newstreamptr, const char *restrict filename,
                  const char *restrict mode, FILE *restrict stream);  // since C11
...
| timespec   | since C11 | **structure** holding an interval broken down into seconds and nanoseconds |
