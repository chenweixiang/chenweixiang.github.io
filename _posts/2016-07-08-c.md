...
| Type | Format Specifier | Standard | Size |
| :--- | :--------------- | :------- | :--- |
| char | ```%c``` | | CHAR_BIT bits |
| signed char | ```%c``` (or ```%hhi``` for numerical output) | | same size as char |
| unsigned char | ```%c``` (or ```%hhu``` for numerical output) | | same size as char |
| short<br>short int<br>signed short<br>signed short int | ```%hi``` | | at least 16 bits |
| unsigned short<br>unsigned short int | ```%hu``` | | similar to short |
| int<br>signed<br>signed int| ```%i``` or ```%d``` | | at least 16 bits |
| unsigned<br>unsigned int | ```%u``` | | similar to int |
| long<br>long int<br>signed long<br>signed long int | ```%li``` | | at least 32 bits |
| unsigned long<br>unsigned long int | ```%lu``` | | similar to long |
| long long<br>long long int<br>signed long long<br>signed long long int | ```%lli``` | since C99 | at least 64 bits |
| unsigned long long<br>unsigned long long int | ```%llu``` | since C99 | similar to long long |
| float | ```%f``` | | |
| double | ```%f``` ```%F```<br>(```%lf``` ```%lF``` for ```scanf()```)<br>```%g``` ```%G``` ```%e``` ```%E``` (for scientific notation) | | |
| long double | ```%Lf``` ```%LF```<br>```%Lg``` ```%LG```<br>```%Le``` ```%LE``` | | |
