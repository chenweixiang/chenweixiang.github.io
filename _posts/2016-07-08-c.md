...
| `!` `~` `++` `--` `+` `-` `*` `(type)` `sizeof`         | **right to left** | Unary operators, increment operator, decrement operator, type cast, sizeof |
...
* There are a few situations where *goto* may find a place. The most common is to abandon processing in some deeply nested structure, such as breaking out of two or more loops at once.
